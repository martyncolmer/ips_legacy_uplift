...
def do_ips_imbweight_calculation(surveyData                   # Input
#                               , output                    # Output
#                               , summary                   # Summary
...
#    sys.exit() 
    ...
    df_total_traffic = df_output_data[["PORTROUTE", "FLOW"]].copy()
    df_total_traffic["TOT_NI_TRAFFIC"] = pd.Series(df_output_data["NON_RESPONSE_WT"] 
                                                   * df_output_data["MINS_WT"]            
                                                   * df_output_data["TRAFFIC_WT"]        
                                                   * df_output_data["UNSAMP_TRAFFIC_WT"])
...
#    sys.exit()
    ...
    flow_condition = (df_output_data["FLOW"] == 1) | (df_output_data["FLOW"] == 5)
    arrivedepart_condition = df_output_data["ARRIVEDEPART"] == 2
    df_output_data.loc[(flow_condition) & (arrivedepart_condition)
                         , "IMBAL_WT"] = df_output_data["IMBAL_PORT_FACT_PV"]
...
#    print df_survey_data[["FLOW","ARRIVEDEPART","IMBAL_WT","IMBAL_PORT_FACT_PV"]]
    flow_condition = (df_output_data["FLOW"] == 3) | (df_output_data["FLOW"] == 7)
    arrivedepart_condition = df_output_data["ARRIVEDEPART"] == 1
    df_output_data.loc[(flow_condition) & (arrivedepart_condition)
                         , "IMBAL_WT"] = df_output_data["IMBAL_PORT_FACT_PV"]
...
#    sys.exit()
    ...
    df_output_data.loc[(flow_condition)
                         , "IMBAL_WT"] = (df_output_data["IMBAL_WT"]
                                           * df_output_data["IMBAL_CTRY_FACT_PV"])
...
#    csv[["SERIAL", "AGE", "APORTLATDEG", "APORTLATMIN", "SHIFT_WT", "IMBAL_WT"]].to_csv(r"H:\My Documents\Documents\Git Repo\Misc and Admin\LegacyUplift\Compare\SAS")
#    df_prepost[["SERIAL", "AGE", "APORTLATDEG", "APORTLATMIN", "SHIFT_WT", "IMBAL_WT"]].to_csv(r"H:\My Documents\Documents\Git Repo\Misc and Admin\LegacyUplift\Compare\DF")
#    sys.exit()
#    print "SAS, prepost.sas7bdat"
#    print csv["SHIFT_WT"].head()
#    print "DF, prepost.sas7bdat"
#    print df_prepost["SHIFT_WT"].head()
...
#    sys.exit()
    ...
    df_output_data.loc[((df_output_data.PORTROUTE.isin(df_calc_departures.PORTROUTE)) 
                        & (df_output_data["FLOW"].isin(flow_range))), "IMBAL_WT"] \
...
#    sys.exit()
    ...
    serial_range = list(df_output_data["SERIAL"])
    serial_condition = gldf_survey_data["SERIAL"].isin(serial_range)
    gldf_survey_data.loc[~serial_condition, "IMBAL_WT"] = 1
...
#    sys.exit()
    ...
    gldf_survey_data["SUM_PRIOR_WT"] = pd.Series(gldf_survey_data["SHIFT_WT"]
                                          * gldf_survey_data["NON_RESPONSE_WT"]
                                          * gldf_survey_data["MINS_WT"]
                                          * gldf_survey_data["TRAFFIC_WT"]
                                          * gldf_survey_data["UNSAMP_TRAFFIC_WT"])
    gldf_survey_data["SUM_IMBAL_WT"] = pd.Series(gldf_survey_data["IMBAL_WT"]
                                           * gldf_survey_data["SHIFT_WT"]
                                           * gldf_survey_data["NON_RESPONSE_WT"]
                                           * gldf_survey_data["MINS_WT"]
                                           * gldf_survey_data["TRAFFIC_WT"]
                                           * gldf_survey_data["UNSAMP_TRAFFIC_WT"])
    
    df_sliced = gldf_survey_data[gldf_survey_data["SUM_PRIOR_WT"] > 0]
    df_summary_data = df_sliced.groupby(["FLOW"]).agg({\
            'SUM_PRIOR_WT':'sum'
            ,'SUM_IMBAL_WT':'sum'})
...
def calculate_imbalance_weight(surveyData                   # Input
#                               , output                    # Output
#                               , summary                   # Summary
                               , var_serialNum, var_shiftWeight, var_NRWeight
                               , var_minWeight, var_trafficWeight, var_OOHWeight
...
#    # Call JSON configuration file for error logger setup
    ...
    df_imbalance_calculated = do_ips_imbweight_calculation(surveyData   # Input
#                               , output                                # Output
#                               , summary                               # Summary
                               , var_serialNum, var_shiftWeight, var_NRWeight
                               , var_minWeight, var_trafficWeight, var_OOHWeight
...
if __name__ == "__main__":
    calculate_imbalance_weight(surveyData = "SAS_SURVEY_SUBSAMPLE" # Input
#                               , output                            # Output
#                               , summary                            # Summary
