...
def do_ips_regional_weight_calculation(df_input_data, var_serial, var_final_weight):
    ...
    for level in range(1, MAXIMUM_LEVEL + 1):
        ...
        df_impute_towns_ext_mod['FIN'] = df_impute_towns_ext_mod[var_final_weight] * df_impute_towns_ext_mod[
            VISIT_WEIGHT_VARIABLE]
        df_impute_towns_ext_mod['STY'] = df_impute_towns_ext_mod[STAY_VARIABLE] * df_impute_towns_ext_mod[
            var_final_weight] * df_impute_towns_ext_mod[STAY_WEIGHT_VARIABLE]
        df_impute_towns_ext_mod['EXP'] = df_impute_towns_ext_mod[SPEND_VARIABLE] * df_impute_towns_ext_mod[
            var_final_weight] * df_impute_towns_ext_mod[EXPENDITURE_WEIGHT_VARIABLE]
        ...
        if record_count > 0:
            ...
            if level < 4:
                ...
                condition = ((trunc_segment[level - 1]['TOWN_COUNT'] >= 20)
                            ...
                            'UNKNOWN_FINAL_WEIGHT']) / trunc_segment[level - 1]['KNOWN_FINAL_WEIGHTS']) <= 2)
