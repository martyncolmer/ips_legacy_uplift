...
connection_string = f"mssql+pyodbc://{username}:{password}@IPS"
...
def select_data(column_name: str, table_name: str, condition1: str, condition2: str) -> Optional[pandas.DataFrame]:
    ...
    try:
        return pandas.read_sql_query(sql, con=connection_string)
    except Exception as err:
        print(err)
...
def insert_dataframe_into_table(table_name: str, dataframe: pandas.DataFrame) -> None:
    ...
    try:
        dataframe.to_sql(table_name, con=connection_string, if_exists='append',
